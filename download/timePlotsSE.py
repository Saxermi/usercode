...
df = pd.read_csv('/t3home/frejalom/cmssw/CMSSW_15_0_0_pre2/src/usercode/download/summary_stats_time_60.csv')
df_final = df[
...
for _, row in df_final.iterrows():
    # ax.errorbar(
    #     row['Beta1'], row['Mean'], yerr=row['Std'],
    #     fmt=marker_map[row['Group']], color=color_map[row['EventType']],
    #     ecolor=color_map[row['EventType']], markersize=10 
    # )
    ax.plot(
        row['Beta1'], row['Mean'],
...
ax.set_ylabel('Mean Time for Clustering [s]', fontsize=12)
ax.set_title('Inlfuence of firstbestastop on Time to Cluster', fontsize=14)
ax.set_xticks(categories)
ax.legend(title='SE Types & Algorithm', bbox_to_anchor=(1.05, 1), loc='upper left')

plt.tight_layout()
fname = f'timeVsFirstbetastopNoErrors.png'
plt.savefig(fname)
plt.close()
print(f"✅ Saved {fname}")
