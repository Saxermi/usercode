...
class SetupRunDirectoryTask(law.Task):
    ...
    base_dir = law.LocalParameter(default="/work/msaxer/ba")
...
class ExtractParametersTask(law.Task):
    ...
    cpp_file = law.LocalParameter(default="/path/to/your_file.cpp")
...
class RunJobTask(law.Task):
    ...
    def run(self):
        ...
        bash_script = "pvslurmmaster_notifyme.bsh" if self.notify else "pvslurmmaster.bsh"
        run_dir = open(self.input().path).read().strip()
        overlap_dir = f"n{int(abs(self.overlap)*10)*10}" if self.overlap < 0 else f"{int(self.overlap*10)*10}"
        job_dir = os.path.join(run_dir, self.sample, overlap_dir, str(self.blocksize))
        if not os.path.exists(job_dir):
...
class PlottingTask(law.Task):
    ...
    def run(self):
        plot_script = r"""
import ROOT, os, re
# --- Begin ROOT plotting code ---
print("Running ROOT plotting code...")
# (Insert your full ROOT plotting script here.)
# For example, you would open your ROOT files, loop over histograms, draw and save them.
# --- End ROOT plotting code ---
"""
        plot_file = "plot_script.py"
...
class ArchiveOutTask(law.Task):
    ...
    def run(self):
        source_dir = "/t3home/msaxer/cmssw/CMSSW_14_2_0_pre4/src/usercode/PrimaryVertexAnalyzer/test/jobs"
        run_dir = open(self.input().path).read().strip()
        archive_dir = os.path.join(run_dir, "outarchive")
