...
mapping = {k: i for i, k in enumerate(order_of_checkpoints)}
averaged_df['sort_order'] = averaged_df['Checkpoint'].map(mapping)
averaged_df = averaged_df.sort_values(by='sort_order')
averaged_df.drop('sort_order', axis=1, inplace=True)
...
plt.title("Time taken for each step (n=%d, Earliest: %s)" % (n_files, earliest_date.strftime('%Y-%m-%d %H:%M:%S')))
...
plt.ylabel("Processing Steps")
plt.title("Size after each step (n=%d, Earliest: %s)" % (n_files, earliest_date.strftime('%Y-%m-%d %H:%M:%S')))
plt.gca().invert_yaxis()
...
plt.title("Time and Size after each step (n=%d, Earliest: %s)" % (n_files, earliest_date.strftime('%Y-%m-%d %H:%M:%S')))
