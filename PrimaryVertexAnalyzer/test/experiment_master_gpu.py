...
base_path = "experimental_run_200"
...
def submit_job(sample, overlap, blocksize, iterating_blocksize=False, notify=False):
    ...
    bash_script = "pvslurmmaster_notifyme_gpu.bsh"  # if notify else "pvslurmmaster.bsh"

    # Calculate the directory name based on overlap (convert to an integer percentage)
    overlap_dir = (
        int(abs(overlap) * 10) * 10
    ...
    try:
        ...
        logging.info(
            f"Job for {sample} with overlap {overlap} and blocksize {blocksize} submitted successfully."
        )
        print(result.stdout)
        print(
            f"Job for {sample} with overlap {overlap} and blocksize {blocksize} submitted successfully."
        ...
        logging.error(
            f"Failed to submit job for {sample} with overlap {overlap} and blocksize {blocksize}. Error: {e}"
        )
        print(
            f"Failed to submit job for {sample} with overlap {overlap} and blocksize {blocksize}. Error: {e}"
        )
...
def main():
    ...
    overlaps = [0.50]  # Add negative overlaps here
    print("running")
    ...
    if TEST_MODE:
        ...
        for idx, (sample, overlap, blocksize) in enumerate(
            itertools.product(subsets, overlaps, blocksizes)
        ):
            ...
            time.sleep(1)
            submit_job(
                sample, overlap, blocksize, iterating_blocksize=True, notify=False
            )
